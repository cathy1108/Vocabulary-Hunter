...
const getEnv = (key) => {
  try {
    ...
    // 某些環境下可能直接定義在 window 或 __env
    if (typeof window !== 'undefined' && window[key]) {
      return window[key];
    }
  } catch (e) {
    console.warn(`無法讀取環境變數 ${key}`);
  }
  return "";
};
...
const localFirebaseConfig = {
  // 安全讀取 API Key，避免 process undefined 錯誤
...
const getFirebaseConfig = () => {
  try {
    ...
    console.warn("系統環境變數解析失敗，使用 localFirebaseConfig");
  }
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(wordsRef, (snapshot) => {
      ...
      setErrorMsg("同步失敗，請檢查 Firebase 權限。");
    });
    return () => unsubscribe();
  }, [user]);
  ...
  const fetchTranslation = async () => {
    ...
    // 使用安全讀取函數獲取 Gemini Key
    const apiKey = process.env.REACT_APP_GEMINI_KEY;
    
    if (!apiKey) {
      setErrorMsg("未偵測到 API Key。如果您在本地執行，請確保已設定環境變數或直接填入 Key。");
    ...
    try {
      const translatePrompt = `將${langMode === 'EN' ? '英文' : '日文'}單字 "${newWord.term}" 翻譯成繁體中文，給出最簡短的一個意思，不要有額外解釋。`;
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error?.message || `請求失敗: ${res.status}`);
      }
  ...
  return (
    <div className={`min-h-screen ${colors.bg} text-stone-800 pb-32 font-sans relative`}>
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8">
        {errorMsg && (
          <div className="mb-6 p-4 bg-red-50 border-2 border-red-100 text-red-700 rounded-2xl flex items-center gap-3 font-bold animate-in fade-in zoom-in duration-300">
            <AlertCircle size={20} />
            {errorMsg}
          </div>
        )}
      ...
      <div className="fixed bottom-10 left-0 right-0 px-6 z-20">
        <div className="max-w-4xl mx-auto bg-white/95 backdrop-blur-md border border-stone-200 p-5 rounded-[30px] shadow-2xl flex items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <span className="font-black text-stone-700">進度：{Math.round(archivePercentage)}%</span>
          </div>
